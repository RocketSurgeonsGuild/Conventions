...
metadata:
    ...
    source: security-review:commands/security-review-staged.md
---
...
2. **Identify Changed Files**:
    - You **MUST** execute the `.specify/scripts/bash/detect-changed-files.sh` with `--json` to detect changed files in the working directory (`Mode B`).
...
1. **Identify Scope**: Run `.specify/scripts/bash/detect-changed-files.sh` to identify the changed files.
