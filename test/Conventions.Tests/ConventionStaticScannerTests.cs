...
public class ConventionStaticScannerTests : AutoFakeTest
{
    ...
    {
        var scanner = AutoFake.Resolve<ConventionContextBuilder>().WithConventionsFrom(Imports.Instance);
    ...
    public void ShouldBuildAProvider()
    {
        var scanner = ConventionContextHelpers.CreateProvider(
            new ConventionContextBuilder(new Dictionary<object, object?>()).WithConventionsFrom(Imports.Instance),
