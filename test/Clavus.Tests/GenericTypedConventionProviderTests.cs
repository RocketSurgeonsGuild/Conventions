...
using Rocket.Surgery.Clavus.Tests.Fixtures;
...
public class GenericTypedClavusProviderTests()
    : AutoFakeTest<TestRecord>(TestRecord.Create(LogEventLevel.Information))
{
    ...
    {
        var b = new B();
        var d1 = new ServicePart((_, _, _) => { });
        var d2 = new ServicePart((_, _, _) => { });
        var d3 = new ServicePart((_, _, _) => { });
        var c = new C();
        var d = new D();
        var e = new E();
        var f = new F();
    ...
    {
        var b = new B();
        var d1 = new ClavusOrDelegate(new ServicePart((_, _, _) => { }), 0, ClavusCategory.Application);
        var d2 = new ClavusOrDelegate(new ServicePart((_, _, _) => { }), int.MinValue, ClavusCategory.Core);
        var d3 = new ClavusOrDelegate(new ServicePart((_, _, _) => { }), int.MaxValue, new("Custom"));
        var c = new C();
        var d = new D();
        var e = new E();
        var f = new F();
    ...
    private SettingsTask VerifyWithParameters(ClavusProvider provider, HostType hostType, ImmutableArray<ClavusCategory> categories) =>
